...
public class HintsOfCodeGenOnMySqlDaoUnitTest {
	...
	public void setUp() throws Exception {
		...
		List<HintsOfCodeGenOnMySql> daoPojos1 = new ArrayList<HintsOfCodeGenOnMySql>(
				3);

		for (int i = 0; i < 3; i++) {
			HintsOfCodeGenOnMySql daoPojo = new HintsOfCodeGenOnMySql();
			daoPojo.setAge(i + 20);
		...
		List<HintsOfCodeGenOnMySql> daoPojos2 = new ArrayList<HintsOfCodeGenOnMySql>(
				3);
		for (int i = 0; i < 3; i++) {
			HintsOfCodeGenOnMySql daoPojo = new HintsOfCodeGenOnMySql();
			daoPojo.setAge(i + 20);
...
//	}
	...
	public void testtest_build_query_allShard_async() throws Exception {
		Integer Age = 20;// Test value here
		DalHints hints = new DalHints();
		List<HintsOfCodeGenOnMySql> ret = dao
				.test_build_query_allShard_async(Age, hints);
		assertNull(ret);
		Future<List<HintsOfCodeGenOnMySql>> fr = (Future<List<HintsOfCodeGenOnMySql>>) hints
				.getAsyncResult();
		ret = fr.get(); // 异步返回结果
	...
	public void testtest_build_query_shards_callback() throws Exception {
		...
		callback.waitForDone();
		ret = (List<HintsOfCodeGenOnMySql>) callback.getResult(); // 异步返回结果
