(function(window, undefined) {
	...
	var refreshMember = function() {
		...
		cblock($("body"));
		$.get(
				"/rest/member/groupuser?groupId=" + current_group + "&rand="
	...
	var reload_all_groups = function() {
		...
		$.get("/rest/group/get?rand=" + Math.random()).done(
				function(data) {
					if ($("#group_list")[0] != undefined
							&& $("#group_list")[0].selectize != undefined) {
	...
	var reload_all_members = function() {
		...
		$.get("/rest/member/all?rand=" + Math.random()).done(
				function(data) {
					if ($("#members")[0] != undefined
							&& $("#members")[0].selectize != undefined) {
					...
					.jstree(
							{
								'core' : {
									...
									'data' : {
										'url' : function(node) {
											return node.id == "#" ? "/rest/member?root=true&rand="
													+ Math.random()
													: "/rest/member?rand="
															+ Math.random();
										}
