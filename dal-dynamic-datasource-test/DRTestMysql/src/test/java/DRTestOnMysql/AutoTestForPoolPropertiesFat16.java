 ...
 */
public class AutoTestForPoolPropertiesFat16 {
    ...
    private static Logger log = LoggerFactory.getLogger(AutoTestForPoolPropertiesFat16.class);
...
//                toBeContinued = true;
    ...
    public void checkAfterMHASwitch(DRTestDao dao, String hostname, DalHints hints) {
        //检查切换是否生效
        ...
        while (toBeContinued) {
            try {
                ...
                assertNotEquals(hostname, queryHostName(keyName1, hints, dao));
                toBeContinued = false;
                log.info(String.format("第 %d 次请求成功 ", times));
...
//      检查连接串切换是否生效
        checkAfterMHASwitch(dao, currentHostname1, null);
    ...
    public void modifyDatasourcePropertiesAndConnectionStringWithMultipleKeys() throws Exception {
        ...
        connectionStringSwitch.postByMHA(isPro);
//      检查连接串切换是否生效
        checkAfterMHASwitch(dao1, currentHostname1, new DalHints().inShard(0));
...
//        map.put("validationQueryTimeout", "5");//原来没有这个设置，默认值为5
        ...
        map.put("connectionProperties", "sendTimeAsDateTime=false;sendStringParametersAsUnicode=false;rewriteBatchedStatements=true;allowMultiQueries=true;useUnicode=true;characterEncoding=UTF-8;useSSL=false");
        //原来没有设置
