(function(window, undefined) {
	...
	var refreshDB = function() {
		...
		cblock($("body"));
		$.get(
				"/rest/groupdb/groupdb?groupId=" + current_group + "&rand="
	...
	var transferDB = function() {
		...
		if (record != null) {
			$
					.get("/rest/groupdb?root=true&rand=" + Math.random())
					.done(
							function(data) {
								if ($("#transferGroup")[0] != undefined
										&& $("#transferGroup")[0].selectize != undefined) {
									$("#transferGroup")[0].selectize
											.clearOptions();
								} else {
					...
					.jstree(
							{
								'core' : {
									...
									'data' : {
										'url' : function(node) {
											return node.id == "#" ? "/rest/groupdb?root=true&rand="
													+ Math.random()
													: "/rest/groupdb?rand="
															+ Math.random();
										}
