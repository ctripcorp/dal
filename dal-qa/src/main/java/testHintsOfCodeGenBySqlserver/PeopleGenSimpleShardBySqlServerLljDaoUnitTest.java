...
/**
 * JUnit test of PeopleGenSimpleShardBySqlServerLljDao class. Before run the
 * unit test, you should initiate the test data and change all the asserts
 * correspond to you case.
 **/
public class PeopleGenSimpleShardBySqlServerLljDaoUnitTest {

	private static final String DATA_BASE = "testHintsOfCodeGenBySqlserver";

	private static DalClient client = null;
	private static PeopleGenSimpleShardBySqlServerLljDao dao = null;
	...
	public static void setUpBeforeClass() throws Exception {
		...
		dao = new PeopleGenSimpleShardBySqlServerLljDao();
	}
	...
	public void setUp() throws Exception {
		...
		List<PeopleGenSimpleShardBySqlServerLlj> daoPojos1 = new ArrayList<PeopleGenSimpleShardBySqlServerLlj>(
				3);
		for (int i = 0; i < 3; i++) {
			PeopleGenSimpleShardBySqlServerLlj daoPojo = new PeopleGenSimpleShardBySqlServerLlj();
		...
		List<PeopleGenSimpleShardBySqlServerLlj> daoPojos2 = new ArrayList<PeopleGenSimpleShardBySqlServerLlj>(
				3);
		for (int i = 0; i < 3; i++) {
			PeopleGenSimpleShardBySqlServerLlj daoPojo = new PeopleGenSimpleShardBySqlServerLlj();
			//daoPojo.setPeopleID(Long.valueOf(i) + 1);
...
//		List<PeopleGenSimpleShardBySqlServerLlj> list = dao
//				.getAll(new DalHints());
//	}
//
//	@Test
...
//		daoPojo.setCountryID(30);
//		daoPojo.setProvinceID(40);
//		daoPojo.setName("new20");
//		int affected = dao.insert(new DalHints().inShard(1), daoPojo);
////		assertEquals(1, affected);
//		
//		daoPojo.setCityID(21);
//		daoPojo.setCountryID(31);
//		daoPojo.setProvinceID(41);
//		daoPojo.setName("new21");
...
//		int pageSize = 100;
//		int pageNo = 1;
//		List<PeopleGenSimpleShardBySqlServerLlj> list = dao.queryByPage(
...
//	}
	...
	public void testtest_build_query_shards_async() throws Exception {
		...
		List<PeopleGenSimpleShardBySqlServerLlj> ret = dao
				.test_build_query_shards_async(CityID, hints, shards);

		assertNull(ret);
		Future<List<PeopleGenSimpleShardBySqlServerLlj>> fr = (Future<List<PeopleGenSimpleShardBySqlServerLlj>>) hints
				.getAsyncResult();
		ret = fr.get(); // 异步返回结果
	...
	public void testtest_build_query_allShard_callback() throws Exception {
		...
		callback.waitForDone();
		ret = (List<PeopleGenSimpleShardBySqlServerLlj>) callback.getResult(); // 异步返回结果
	...
	public void testtest_def_query_allShard_callback() throws Exception {
		...
		DalHints hints = new DalHints();
		DefaultResultCallback callback = new DefaultResultCallback();
		List<PeopleGenSimpleShardBySqlServerLlj> ret = dao.test_def_query_allShard_callback(CityID, hints,callback);
        assertNull(ret);
	...
	public void testtest_def_query_shards_async() throws Exception {
		...
		List<PeopleGenSimpleShardBySqlServerLlj> ret = dao.test_def_query_shards_async(CityID, hints,shards);
		assertNull(ret);
		Future<List<PeopleGenSimpleShardBySqlServerLlj>> fr = (Future<List<PeopleGenSimpleShardBySqlServerLlj>>) hints
				.getAsyncResult();
		ret = fr.get(); // 异步返回结果
