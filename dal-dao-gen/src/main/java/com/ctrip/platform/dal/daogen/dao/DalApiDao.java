...
public class DalApiDao {
	...
	public List<DalApi> getAllDalApi() {
		return this.jdbcTemplate.query(
				"select id, language, db_type, crud_type, method_declaration"
	...
	public DalApi getDalApiById(Integer id) {
		List<DalApi> apis = this.jdbcTemplate
				.query("select id, language, db_type, crud_type, method_declaration"
						+ ", method_description,sp_type from api_list where id = "
						+ id, new RowMapper<DalApi>() {
					public DalApi mapRow(ResultSet rs, int rowNum)
							throws SQLException {
						return DalApi.visitRow(rs);
					}
	...
	public List<DalApi> getDalApiByLanguageAndDbtype(String language,
			String db_type) {
		List<DalApi> apis = this.jdbcTemplate
				.query("select id, language, db_type, crud_type, method_declaration"
						+ ", method_description,sp_type from api_list where language = ?"
						+ " and db_type=?", new Object[] { language, db_type },
						new RowMapper<DalApi>() {
							public DalApi mapRow(ResultSet rs, int rowNum)
									throws SQLException {
								return DalApi.visitRow(rs);
	...
	public List<DalApi> getDalApiByLanguageAndDbtypeAndSptype(String language,
			String db_type, String sp_type) {
		List<DalApi> apis = this.jdbcTemplate
				.query("select id, language, db_type, crud_type, method_declaration"
						+ ", method_description,sp_type from api_list where language = ?"
						+ " and db_type=? and sp_type=?", new Object[] {
						language, db_type, sp_type }, new RowMapper<DalApi>() {
					public DalApi mapRow(ResultSet rs, int rowNum)
							throws SQLException {
						return DalApi.visitRow(rs);
					}
