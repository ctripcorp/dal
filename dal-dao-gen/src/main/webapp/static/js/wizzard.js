...
(function(window, undefined) {
				...
				function(retValue) {
					...
					if ($("#page1").attr('is_update') == "1"
						...
						if (record.table_names != undefined) {
							$('#table_list').multipleSelect('setSelects',
									record.table_names.split(","));
						}
						if (record.view_names != undefined) {
							$('#view_list').multipleSelect('setSelects',
									record.view_names.split(","));
						}
						if (record.sp_names != undefined) {
							$('#sp_list').multipleSelect('setSelects',
									record.sp_names.split(","));
						}
	...
	var step1_sql = function(record, current) {
		if ($("#sql_class_name")[0] != undefined
				&& $("#sql_class_name")[0].selectize != undefined) {
		...
		if ($("#sql_pojo_name")[0] != undefined
				&& $("#sql_pojo_name")[0].selectize != undefined) {
		...
		$
				.get(
						"/rest/task/sql_class?project_id="
								+ w2ui['grid'].current_project + "&db_name="
								+ $("#databases").val() + "&rand="
								+ Math.random(),
						function(data) {
							...
							$("#sql_class_name")[0].selectize
									.refreshOptions(false);
							$("#sql_pojo_name")[0].selectize
									.refreshOptions(false);

							if (update) {
								if (data.classes.length != 0) {
									$("#sql_class_name")[0].selectize
											.setValue(record['class_name']);
								} else {
									$("#sql_class_name")[0].selectize
											.addOption({
								...
								if (record['scalarType']) {
									$("#free_sql_scalarType").val(
											record['scalarType']);
								} else {
									$("#free_sql_scalarType").val("List");
								}
								$("#free_sql_pagination").prop('checked',
										record['pagination']);
								$("#free_sql_crud_option").val(
										record['crud_type']);
	...
	var step2_1 = function(record, current) {
		...
		if (checkDaoNameConflict($('#table_list').multipleSelect('getSelects')
				.join(","), {
			"prefix" : $("#prefix").val(),
			"suffix" : $("#suffix").val()
		})) {
				...
				.done(
						function(retValue) {
									...
									.each(
											...
											function(index, value) {
												if (value['crud_type']
														.toLowerCase() == 'select') {
													append_api(
															"RetrieveMethodListDiv",
															value);
												} else if (value['crud_type']
														.toLowerCase() == 'update') {
													append_api(
															"UpdateMethodListDiv",
															value);
							...
							if ($("#page1").attr('is_update') == "1"
								...
								$.each(data, function(index, value) {
									if ($.inArray('dal_api_' + value['id'],
											tempArr) == -1) {
										$("#dal_api_" + value['id']).prop(
												"checked", false);
									}
								});
							}
	...
	var methodApiClickHandler = function() {
		...
		bindSelectAllCheck("RetrieveMethodListDiv",
				"SelectAllRetrieveMethodAPIChk");
	...
	var bindSelectAllCheck = function(methodListDiv, selectAllMethodCheckbox) {
		...
		$("#" + methodListDiv + " :checkbox")
				.bind(
						"click",
						function() {
							var totalCkbx = $("#" + methodListDiv
									+ " :checkbox").length;
							var checkedCkbx = 0;
						...
						function(data) {
									...
									.multipleSelect(
											{
												...
												onUncheckAll : function() {
													ace.edit("sql_builder")
															.setValue("");
												},
												onClick : function(option) {
													var tempValue = $('#fields')
															.multipleSelect(
																	'getSelects');
							...
							if ($("#page1").attr('is_update') == "1") {
								if (!$.isEmpty(record["orderby"])) {
									$("#orderby_field").val(
											record["orderby"].split(',')[0]);
									$("#orderby_sort").val(
											record["orderby"].split(',')[1]);
								}
								if (record["scalarType"]) {
									$("#auto_sql_scalarType").val(
											record["scalarType"]);
								} else {
									$("#auto_sql_scalarType").val("List");
								}
								$("#auto_sql_pagination").prop('checked',
										record['pagination']);
							...
							if (op_type == "select") {
								...
								if ($("#sql_style").val() == "csharp") {
									$(
											"#auto_sql_scalarType option[value='Single']")
											.hide();
								} else {
									$(
											"#auto_sql_scalarType option[value='Single']")
	...
	var step2_2_1 = function(record, current) {
		...
		if (crud_option != "delete"
				&& $('#fields').multipleSelect('getSelects').length < 1) {
	...
	var step2_2_2 = function(record, current) {
		...
		postData["fields"] = $('#fields').multipleSelect('getSelects')
				.join(",");
				...
				function(data) {
					if (data.code == "OK") {
						...
						$.each(mockValue, function(index, value) {
							$("#auto_sql_mock_value").append(
									sprintf(mockValueHtml,
											paramNameList[index], value));
						});
						var editor = ace.edit("step2_2_3_sql_editor");
						editor.setTheme("ace/theme/monokai");
						editor.getSession().setMode("ace/mode/sql");
						editor.setValue(ace.edit("step2_2_2_sql_editor")
								.getValue());
	...
	var step2_2_3 = function(record, current) {
		...
		var mockValues = [];
		$.each($("#auto_sql_mock_value").children("div"),
				function(index, value) {
					var first = $(value).children("input").eq(0);
						...
						function(data) {
							if (data.code == "OK") {
								...
								if ($("#crud_option").val() == "select"
									...
									$("#auto_sql_validate_result")
											.html(
													data.info
															+ ". And below is the sql execution plan.");
									var explanJson = $
											.parseJSON(data['explanJson']);
									$("#auto_select_type").html(
											explanJson[0]['select_type']);
									$("#auto_type").html(explanJson[0]['type']);
									$("#auto_possible_keys").html(
											explanJson[0]['possible_keys']);
									$("#auto_key").html(explanJson[0]['key']);
									$("#auto_rows").html(explanJson[0]['rows']);
									$("#auto_extra").html(
											explanJson[0]['extra']);
									$("#auto_sql_validate_result_div > table")
											.show();
								} else {
									$("#auto_sql_validate_result").html(
											data.info);
									$("#auto_sql_validate_result_div > table")
											.hide();
								}
								...
								editor.getSession().setMode("ace/mode/sql");
								editor.setValue(ace
										.edit("step2_2_2_sql_editor")
										.getValue());
								editor.setReadOnly(true);
								current.hide();
								$(".step2-2-4").show();
							} else {
								$.showMsg("error_msg", "SQL测试执行异常，请检查sql及对应参数！"
										+ data.info);
							}
						}).fail(function(data) {
					alert("保存出错！");
	...
	var existKeyword_Nolock = function() {
		if ($("#free_sql_crud_option").val() == "select"
				&& ace.edit("sql_editor").getValue().toLowerCase().indexOf(
						"nolock") == -1
				&& $(".step2-2-1").attr("dbCatalog") != "MySql") {
			...
			if ($("#sql_style").val() == "csharp") {
				if (temp != null && temp != "") {
					htmls = htmls
							...
							+ sprintf(variable_typesHtml, sprintf(
									"id='db_type_%s'", sprintf("param%s", i)));
				} else {
					htmls = htmls
				...
				if (temp != null && temp != "") {
					htmls = htmls
							+ sprintf(variableHtmlOrigin, temp)
							+ sprintf(variable_typesHtml_ForJAVA, sprintf(
									"id='db_type_%s'", sprintf("param%s", i)));
				} else {
					htmls = htmls
							+ sprintf(variableHtmlOrigin, sprintf("param%s", i))
		...
		if ($("#page1").attr('is_update') == "1") {
			...
			$.each(splitedParams, function(index, value) {
				if (index <= i) {
					...
					if ($("#sql_style").val() != "csharp") {
						var chk = $("#db_type_param" + paramIndex).siblings(
								'div').children(":checkbox");
						var sensitive = resultParams[2];
						sensitive = sensitive != undefined ? sensitive
								: 'false';
						if (sensitive == 'false')
		...
		$.post(
				...
				function(data) {
					if (data.code == "OK") {
						...
						editor.setTheme("ace/theme/monokai");
						editor.getSession().setMode("ace/mode/sql");
						editor.setValue(ace.edit("step2_3_1_sql_editor")
								.getValue());
						editor.setReadOnly(true);
						current.hide();
						$(".step2-3-4").show();
	...
	var step2_3_4 = function(record, current) {
		...
		var mockValues = [];
		$.each($("#free_sql_mock_value").children("div"),
				function(index, value) {
					var first = $(value).children("input").eq(0);
					mockValues.push($(first).val());
				});
		...
		$
				.post("/rest/task/sql/sqlValidate", postData)
				.done(
						function(data) {
							if (data.code == "OK") {
								$("#error_msg").empty();
								if ($("#free_sql_crud_option").val() == "select"
										&& $(".step2-2-1").attr("dbCatalog") == "MySql") {
									$("#free_sql_validate_result")
											.html(
													data.info
															+ ". And below is the sql execution plan.");
									var explanJson = $
											.parseJSON(data['explanJson']);
									$("#free_select_type").html(
											explanJson[0]['select_type']);
									$("#free_type").html(explanJson[0]['type']);
									$("#free_possible_keys").html(
											explanJson[0]['possible_keys']);
									$("#free_key").html(explanJson[0]['key']);
									$("#free_rows").html(explanJson[0]['rows']);
									$("#free_extra").html(
											explanJson[0]['extra']);
									$("#free_sql_validate_result_div > table")
											.show();
								} else {
									$("#free_sql_validate_result").html(
											data.info);
									$("#free_sql_validate_result_div > table")
											.hide();
								}
	...
	wizzard.prototype = {
		...
		previous : function(current) {
			...
			if (current.hasClass("step2-1") || current.hasClass("step2-2")
