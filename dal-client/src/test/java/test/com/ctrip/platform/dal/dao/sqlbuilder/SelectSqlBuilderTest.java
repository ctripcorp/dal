...
public class SelectSqlBuilderTest {
	...
	@Test
	public void testMySQLPaginationNewInFirstPage() throws SQLException {
		//with order by
		List<String> in = new ArrayList<String>();
		in.add("12");
		in.add("12");
		...
		String expect_sql = "SELECT `PeopleID`, `Name`, `CityID` FROM `People` "
				+ "WHERE `a` = ? AND `b` in ( ? ) AND `b` LIKE ? AND `c` BETWEEN ? AND ? "
				+ "AND `sss` IS NULL ORDER BY `PeopleID` DESC LIMIT 0, 10";
	...
	@Test
	public void testMySQLPaginationNewNotInFirstPage() throws SQLException {
		//with order by
		List<String> in = new ArrayList<String>();
		in.add("12");
		in.add("12");
		...
		String expect_sql = "SELECT `PeopleID`, `Name`, `CityID` FROM `People` "
				+ "WHERE `a` = ? AND `b` in ( ? ) AND `b` LIKE ? AND `c` BETWEEN ? AND ? "
				+ "AND `sss` IS NULL ORDER BY `PeopleID` DESC LIMIT 10, 10";
	...
	@Test
	public void testSQLServerPaginationNewInFirstPage() throws SQLException {
		//with order by
		List<String> in = new ArrayList<String>();
		in.add("12");
		in.add("12");
