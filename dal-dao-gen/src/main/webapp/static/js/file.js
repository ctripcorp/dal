											...
											.w2sidebar(
													{
														name : 'sidebar',
														img : null,
														topHTML : '<div style="background-color: #eee; padding: 10px 5px 10px 20px; border-bottom: 1px solid silver"><a href="javascript:void(0);" onclick="reloadProjects();"><i class="glyphicon glyphicon-refresh" aria-hidden="true"></i>刷新</a></div>',
														nodes : [ {
															id : 'all_projects',
															text : '所有项目',
														...
														onMenuClick : function(
															...
															case "download":
																cblock($("body"));
																var obj = w2ui['sidebar']
																		.get(event.target);
																var url = "/rest/file/download?id="
																		+ obj.project_id
																		+ "&name="
																		+ obj.relativeName;
																if (obj.project_id == undefined) {
																	url = "/rest/file/download?id="
																			+ obj.id;
	...
	$.get("/rest/project",
			function(data) {
				...
				if (currentUrl.indexOf("#") != -1) {
					var currentFile = currentUrl.substring(currentUrl
							.indexOf("#") + 1);
					...
					if (project_id != undefined && name != undefined) {
						...
						$.get("/rest/file/content?" + currentFile, function(
								data) {
							// var real_data = JSON.parse(data);
							ace.edit("code_editor").setValue(data);
							if (name.match(/cs$/)) {
								ace.edit("code_editor").getSession().setMode(
										"ace/mode/csharp");
			...
			$.each(data, function(index, value) {
				if (value.parent) {
					...
					allNodes.push({
						...
						type : "file",
						onClick : function(event) {
							window.location.href = "file.jsp#id=" + project_id
									+ "&name=" + event.object.relativeName;
							$.get("/rest/file/content?id=" + project_id
									+ "&name=" + event.object.relativeName,
									function(data) {
										// var real_data = JSON.parse(data);
										ace.edit("code_editor").setValue(data);
