...
public class DaoOfUserProject {
    ...
    public List<UserProject> getAllUserProjects() {
        return this.jdbcTemplate.query("SELECT id, project_id, user_no FROM user_project",
    ...
    public List<UserProject> getUserProjectsByUser(String userNo) {
        return this.jdbcTemplate.query("SELECT id, project_id, user_no FROM user_project WHERE user_no = ?",
    ...
    public UserProject getUserProject(int project_id, String userNo) {
        try {
            return this.jdbcTemplate.queryForObject("SELECT id,project_id, user_no FROM user_project WHERE project_id=? AND user_no = ?",
    ...
    public UserProject getMinUserProjectByProjectId(int project_id) {
        try {
            return this.jdbcTemplate.queryForObject("SELECT id,project_id, user_no FROM user_project WHERE id=(SELECT min(id) FROM user_project WHERE project_id=?)",
    ...
    public int deleteUserProject(int project_id) {
        try {
            return this.jdbcTemplate.update("DELETE FROM user_project WHERE project_id=?", new Object[]{project_id});
        } catch (DataAccessException ex) {
            ex.printStackTrace();
            return -1;
        }
