...
public class DaoByTableViewSp {
	...
	public List<GenTaskByTableViewSp> getTasksByProjectId(int iD) {
		try {
			return this.jdbcTemplate
					.query("select id, project_id, server_id, db_name,table_names,sp_names,prefix,suffix,cud_by_sp,pagination from task_table where project_id=?",
							new Object[] { iD },
							new RowMapper<GenTaskByTableViewSp>() {
								public GenTaskByTableViewSp mapRow(ResultSet rs,
	...
	public int insertTask(GenTaskByTableViewSp task) {
		try {
			return this.jdbcTemplate
					.update("insert into task_table ( project_id,server_id,  db_name,table_names,sp_names,prefix,suffix,cud_by_sp,pagination) values (?,?,?,?,?,?,?,?,?)",
							task.getProject_id(), task.getServer_id(),
							task.getDb_name(), task.getTable_names(),task.getSp_names(),
	...
	public int updateTask(GenTaskByTableViewSp task) {
		try {
			return this.jdbcTemplate
					.update("update task_table set project_id=?,server_id=?,  db_name=?, table_names=?,sp_names=?,prefix=?,suffix=?,cud_by_sp=?,pagination=? where id=?",
