...
public class ConfigTemplateResource {
	...
	public List<ConfigTemplate> getAllConfigTemplates() throws Exception {
		List<ConfigTemplate> configTemplates = SpringBeanGetter
				.getConfigTemplateDao().getAllConfigTemplates();
	...
	public ConfigTemplate getConfigTemplateById(@QueryParam("id") String id) {
		...
		ConfigTemplate configTemplate = SpringBeanGetter.getConfigTemplateDao()
	...
	public ConfigTemplate getConfigTemplateByConditions(
		...
		configTemplate = SpringBeanGetter.getConfigTemplateDao()
				.getConfigTemplateByConditions(temp);
		return configTemplate;
	}

	@POST
	@Path("addConfigTemplate")
	public Status addConfigTemplate(@FormParam("configType") String configType,
			@FormParam("langType") String langType,
		...
		SpringBeanGetter.getConfigTemplateDao().insertConfigTemplate(
				configTemplate);
		return status;
	}
	...
	public Status updateConfigTemplate(@FormParam("id") String id,
			@FormParam("configType") String configType,
			@FormParam("langType") String langType,
			@FormParam("template") String template) {
		Status status = Status.OK;
		if (id == null || configType == null || langType == null
				|| template == null) {
			status = Status.ERROR;
			status.setInfo("Null parameters");
			return status;
		...
		configTemplate.setLang_type(lang_type);
		configTemplate.setTemplate(template);
		SpringBeanGetter.getConfigTemplateDao().updateConfigTemplate(
				configTemplate);
		return status;
	}
	...
	public Status deleteConfigTemplate(@FormParam("id") String id) {
		...
		ConfigTemplate configTemplate = new ConfigTemplate();
		configTemplate.setId(templateId);
		SpringBeanGetter.getConfigTemplateDao().deleteConfigTemplate(
				configTemplate);
		return status;
	}
