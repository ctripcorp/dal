...
public class AbstractJavaDataPreparer {
	...
	protected void addDatabaseSet(CodeGenContext codeGenCtx,
			String databaseSetName) {
		DaoOfDatabaseSet daoOfDatabaseSet = SpringBeanGetter
				.getDaoOfDatabaseSet();
		List<DatabaseSet> sets = daoOfDatabaseSet
				.getAllDatabaseSetByName(databaseSetName);
		...
		for (DatabaseSet databaseSet : sets) {
			List<DatabaseSetEntry> entries = daoOfDatabaseSet
					.getAllDatabaseSetEntryByDbsetid(databaseSet.getId());
			...
			for (DatabaseSetEntry entry : entries) {
				contextHost.addResource(new Resource(entry
						.getConnectionString()));
			}
		}
	}

	protected JavaTableHost buildTableHost(CodeGenContext codeGenCtx,
			GenTaskByTableViewSp tableViewSp, String tableName)
			throws Exception {
		JavaCodeGenContext ctx = (JavaCodeGenContext) codeGenCtx;
		if (!DbUtils.tableExists(tableViewSp.getAllInOneName(), tableName)) {
			throw new Exception(String.format(
					"The table[%s,%s] doesn't exist, pls check",
		...
		List<AbstractParameterHost> allColumnsAbstract = DbUtils
				.getAllColumnNames(
						tableViewSp.getAllInOneName(),
						tableName,
						new JavaColumnNameResultSetExtractor(tableViewSp
								.getAllInOneName(), tableName));
		if (null == allColumnsAbstract) {
			throw new Exception(String.format(
					"The column names of tabel[%s, %s] is null",
					tableViewSp.getAllInOneName(), tableName));
		}
		...
		List<GenTaskBySqlBuilder> currentTableBuilders = filterExtraMethods(
				ctx, tableViewSp.getAllInOneName(), tableName);
		List<JavaMethodHost> methods = buildSqlBuilderMethodHost(allColumns,
		...
		if (tableHost.isSp()) {
			tableHost.setSpInsert(getSpaOperation(
					tableViewSp.getAllInOneName(), tableName, "i"));
			tableHost.setSpUpdate(getSpaOperation(
					tableViewSp.getAllInOneName(), tableName, "u"));
			tableHost.setSpDelete(getSpaOperation(
					tableViewSp.getAllInOneName(), tableName, "d"));
		}
		return tableHost;
	}

	protected DatabaseCategory getDatabaseCategory(String allInOneName)
			throws Exception {
	...
	protected String getPojoClassName(String prefix, String suffix,
			String tableName) {
	...
	private List<GenTaskBySqlBuilder> filterExtraMethods(
			CodeGenContext codeGenCtx, String allInOneName, String tableName) {
		JavaCodeGenContext ctx = (JavaCodeGenContext) codeGenCtx;
	...
	private List<JavaMethodHost> buildSqlBuilderMethodHost(
		...
		methods.addAll(buildSelectMethodHosts(allColumns,
				currentTableSqlBuilders));
		methods.addAll(buildDeleteMethodHosts(allColumns,
				currentTableSqlBuilders));
		methods.addAll(buildInsertMethodHosts(allColumns,
				currentTableSqlBuilders));
		methods.addAll(buildUpdateMethodHosts(allColumns,
				currentTableSqlBuilders));
		return methods;
	}

	private String buildSelectFieldExp(GenTaskBySqlBuilder sqlBuilder)
			throws Exception {
	...
	private List<JavaMethodHost> buildSelectMethodHosts(
			List<JavaParameterHost> allColumns,
		...
		for (GenTaskBySqlBuilder builder : currentTableBuilders) {
			...
			if (orderBy != null && !orderBy.trim().isEmpty()
				...
				String odyExp = "\"" + str[0] + "\", ";
				odyExp = "asc".equalsIgnoreCase(str[1]) ? odyExp + "true"
						: odyExp + "false";
				method.setOrderByExp(odyExp);
			}
			// select sql have select field and where condition clause
			List<AbstractParameterHost> paramAbstractHosts = DbUtils
					.getSelectFieldHosts(builder.getAllInOneName(),
							builder.getSql_content(),
			...
			method.setParameters(buildMethodParameterHost4SqlConditin(builder,
					allColumns));
	...
	private List<JavaMethodHost> buildDeleteMethodHosts(
			List<JavaParameterHost> allColumns,
			List<GenTaskBySqlBuilder> currentTableBuilders) throws Exception {
		List<JavaMethodHost> methods = new ArrayList<JavaMethodHost>();
	...
	private List<JavaMethodHost> buildInsertMethodHosts(
			List<JavaParameterHost> allColumns,
			List<GenTaskBySqlBuilder> currentTableBuilders) throws Exception {
		List<JavaMethodHost> methods = new ArrayList<JavaMethodHost>();
		...
		for (GenTaskBySqlBuilder builder : currentTableBuilders) {
			...
			if (conditions != null) {
				...
				for (String field : temp) {
					sensitive.put(field.split(",")[0],
							Boolean.parseBoolean(field.split(",")[4]));
				}
			}
			for (String field : fields) {
				for (JavaParameterHost pHost : allColumns) {
					if (pHost.getName().equals(field)) {
						pHost.setSensitive(sensitive.get(field) == null ? false
								: sensitive.get(field));
	...
	private List<JavaMethodHost> buildUpdateMethodHosts(
			List<JavaParameterHost> allColumns,
			List<GenTaskBySqlBuilder> currentTableBuilders) throws Exception {
		List<JavaMethodHost> methods = new ArrayList<JavaMethodHost>();
	...
	private List<JavaParameterHost> buildMethodParameterHost4SqlConditin(
			GenTaskBySqlBuilder builder, List<JavaParameterHost> allColumns) {
		List<JavaParameterHost> parameters = new ArrayList<JavaParameterHost>();
			...
			for (JavaParameterHost pHost : allColumns) {
				if (pHost.getName().equals(name)) {
					...
					if (host_ls.getConditionType() == ConditionType.Between) {
						...
						boolean nullable = tokens.length >= 5 ? Boolean
								.valueOf(tokens[4]) : false;
						host_ls.setNullable(nullable);
						host_bw.setNullable(nullable);
						boolean sensitive = tokens.length >= 6 ? Boolean
								.valueOf(tokens[5]) : false;
						host_ls.setSensitive(sensitive);
						host_bw.setSensitive(sensitive);
					} else {
						boolean nullable = tokens.length >= 4 ? Boolean
								.valueOf(tokens[3]) : false;
						host_ls.setNullable(nullable);
						boolean sensitive = tokens.length >= 5 ? Boolean
								.valueOf(tokens[4]) : false;
						host_ls.setSensitive(sensitive);
					}
					break;
