...
//		}
		...
		List<NewHintsOfCodeGenOnSqlServer> daoPojos1 = new ArrayList<NewHintsOfCodeGenOnSqlServer>(
				3);
		for (int i = 0; i < 3; i++) {
			NewHintsOfCodeGenOnSqlServer daoPojo = new NewHintsOfCodeGenOnSqlServer();
		...
		List<NewHintsOfCodeGenOnSqlServer> daoPojos2 = new ArrayList<NewHintsOfCodeGenOnSqlServer>(
				3);
		for (int i = 0; i < 3; i++) {
			NewHintsOfCodeGenOnSqlServer daoPojo = new NewHintsOfCodeGenOnSqlServer();
			//daoPojo.setPeopleID(Long.valueOf(i) + 1);
	...
	private NewHintsOfCodeGenOnSqlServer createPojo(int index) {
		NewHintsOfCodeGenOnSqlServer daoPojo = new NewHintsOfCodeGenOnSqlServer();
	...
	private void changePojo(NewHintsOfCodeGenOnSqlServer daoPojo) {
		// Change a field to make pojo different with original one
	}
	
	private void changePojos(List<NewHintsOfCodeGenOnSqlServer> daoPojos) {
		for(NewHintsOfCodeGenOnSqlServer daoPojo: daoPojos)
			changePojo(daoPojo);
	}
	
	private void verifyPojo(NewHintsOfCodeGenOnSqlServer daoPojo) {
		//assert changed value
	}
	
	private void verifyPojos(List<NewHintsOfCodeGenOnSqlServer> daoPojos) {
		for(NewHintsOfCodeGenOnSqlServer daoPojo: daoPojos)
			verifyPojo(daoPojo);
	}
...
//		DalHints hints = new DalHints();
//		List<NewHintsOfCodeGenOnSqlServer> daoPojos = dao.queryAll(new DalHints().inShard(0));
...
//	}
	...
	public void testtest_build_query_allShard_callback() throws Exception {
		...
		List<NewHintsOfCodeGenOnSqlServer> ret = dao
				.test_build_query_allShard_callback(CityID, hints, callback);

		assertNull(ret);
		
		callback.waitForDone();
		ret = (List<NewHintsOfCodeGenOnSqlServer>) callback.getResult(); // 异步返回结果
	...
	public void testtest_build_query_shards_async() throws Exception {
		...
		assertNull(ret);
		Future<List<NewHintsOfCodeGenOnSqlServer>> fr = (Future<List<NewHintsOfCodeGenOnSqlServer>>) hints
				.getAsyncResult();
		ret = fr.get(); // 异步返回结果
		assertEquals(2, ret.size());
	}
	...
	public void testtest_def_query_allShard_callback() throws Exception {
		...
		DalHints hints = new DalHints();
		DefaultResultCallback callback = new DefaultResultCallback();
		List<NewHintsOfCodeGenOnSqlServer> ret = dao.test_def_query_allShard_callback(CityID, hints,callback);
        assertNull(ret);
	...
	public void testtest_def_query_shards_async() throws Exception {
		...
		List<NewHintsOfCodeGenOnSqlServer> ret = dao.test_def_query_shards_async(CityID, hints,shards);
		assertNull(ret);
		Future<List<NewHintsOfCodeGenOnSqlServer>> fr = (Future<List<NewHintsOfCodeGenOnSqlServer>>) hints
				.getAsyncResult();
		ret = fr.get(); // 异步返回结果
	...
	public void testtest_def_query_shards_varchar() throws Exception {
		...
		List<NewHintsOfCodeGenOnSqlServer> ret = dao.test_def_query_shards_varchar(name, hints,shards);
		assertNull(ret);
		Future<List<NewHintsOfCodeGenOnSqlServer>> fr = (Future<List<NewHintsOfCodeGenOnSqlServer>>) hints
				.getAsyncResult();
		ret = fr.get(); // 异步返回结果
