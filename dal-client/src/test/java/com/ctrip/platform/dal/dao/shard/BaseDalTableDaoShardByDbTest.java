...
public abstract class BaseDalTableDaoShardByDbTest {
    ...
    public void testInsertSingleWithPkInsertBack() throws SQLException {
        ...
        for (int i = 0; i < mod; i++) {
            ...
            model.setAddress("CTRIP" + j++);
            res = dao.insert(
                    new DalHints().setShardColValue("index", i).setIdentityBack().setKeyHolder(new KeyHolder()), model);
            assertTrue(model.getId() > 0);
            assertEquals(dao.queryByPk(model, new DalHints().setShardColValue("index", i)).getAddress(),
                    model.getAddress());

            // By shardColValue
            model.setId(-1);
            model.setAddress("CTRIP" + j++);
            dao.insert(new DalHints().setShardColValue("tableIndex", i).setIdentityBack().setKeyHolder(new KeyHolder()),
                    model);
            assertTrue(model.getId() > 0);
            assertEquals(dao.queryByPk(model, new DalHints().setShardColValue("tableIndex", i)).getAddress(),
                    model.getAddress());
    ...
    public void testCrossShardInsertPkInsertBack() {
        if (!diff.supportInsertValues || !INSERT_PK_BACK_ALLOWED)
            return;
