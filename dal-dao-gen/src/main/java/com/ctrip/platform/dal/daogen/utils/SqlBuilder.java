...
public class SqlBuilder {
	 ...
	 * @throws Exception
	 */
	public static String pagingQuerySql(String sql, DatabaseCategory dbType,
			CurrentLanguage lang) throws Exception {
		String sql_content = sql.replace("@", ":");
		boolean withNolock = StringUtils.containsIgnoreCase(sql_content,
				"WITH (NOLOCK)");
		if (withNolock)
		...
		try {
			...
			if (dbType == DatabaseCategory.MySql) {
				...
				sb.append(lang == CurrentLanguage.Java ? mysqlPageClausePattern
						: mysqlCSPageClausePattern);
			} else if (dbType == DatabaseCategory.SqlServer) {
				sb.append(plain.toString());
				sb.append(lang == CurrentLanguage.Java ? sqlserverPagingClausePattern
						: sqlseverCSPagingClausePattern);
				/*
				 * List<OrderByElement> orderbys = plain.getOrderByElements();
				 * List<SelectItem> selectitems = plain.getSelectItems(); if
				 * (null == orderbys || orderbys.size() != 1) { throw new
				 * Exception(
				 * "The sql server CET paging must contain one order clause.");
				 * } String rowColumn = "ROW_NUMBER() OVER (ORDER BY " +
				 * orderbys.get(0).toString() + ") AS rownum";
				 * SelectExpressionItem newItem = new SelectExpressionItem( new
				 * Column(rowColumn)); selectitems.add(newItem);
				 * plain.getOrderByElements().clear();
				 * 
				 * String sqlWithRowNum = plain.toString(); if (withNolock) {
				 * sqlWithRowNum = plainSelectToStringAppendWithNoLock(plain); }
				 * 
				 * String cetWrap = "WITH CET AS (" + sqlWithRowNum + ")";
				 * 
				 * selectitems.remove(newItem); List<String> selectField = new
				 * ArrayList<String>(); for (int i = 0; i < selectitems.size();
				 * i++) { if
				 * ("*".equalsIgnoreCase(selectitems.get(i).toString())) {
				 * selectField.add("*"); continue; } SelectExpressionItem item =
				 * (SelectExpressionItem) selectitems .get(i); if
				 * (item.getAlias() != null) {
				 * selectField.add(item.getAlias().getName()); } else { Column
				 * column = (Column) item.getExpression();
				 * selectField.add(column.getColumnName()); } } result = cetWrap
				 * + " SELECT " + StringUtils.join(selectField, ", ") +
				 * " FROM CET WHERE " + (lang == CurrentLanguage.Java ?
				 * sqlserverPageClausePattern : sqlserverCSPageClausePattern);
				 */
			} else {
	...
	private static String plainSelectToStringAppendWithNoLock(PlainSelect plain) {
		...
		if (plain.getFromItem() != null) {
			...
			if (plain.getJoins() != null) {
				...
				while (it.hasNext()) {
					...
					if (join.isSimple()) {
						...
						String temp = join.toString().replace(
			...
			sql.append(PlainSelect.getFormatedList(
					plain.getGroupByColumnReferences(), "GROUP BY"));
			if (plain.getHaving() != null) {
				sql.append(" HAVING ").append(plain.getHaving());
			}
			sql.append(PlainSelect.orderByToString(plain.isOracleSiblings(),
	...
	public static void rebuildJavaInClauseSQL(String sql,
			List<JavaParameterHost> parameters) throws Exception {
	...
	public static void rebuildCSharpInClauseSQL(String sql,
			List<CSharpParameterHost> params) {
