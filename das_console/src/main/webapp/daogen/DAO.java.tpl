...
public class $dao_name extends AbstractDAO {
	...
	#foreach( $method in $methods )
		#set($parameters = $method.getParameters())
   		public #if($method.getAction() == "select")ResultSet#{else}int#end $method.(#foreach($p in $parameters) $p.getType() $p.getName()#if(!{$foreach.count == parameters.sigetMethodNameze()}),#end) {
