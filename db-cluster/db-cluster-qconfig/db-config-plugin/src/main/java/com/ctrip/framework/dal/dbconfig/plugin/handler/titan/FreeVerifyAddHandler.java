 ...
 * <p>
 * Sample:
 * [POST]   http://qconfig.fat16.qa.nt.ctripcorp.com/plugins/titan/freeverify/add?&env=fat&subenv=
 * <p>
...
public class FreeVerifyAddHandler extends BaseAdminHandler implements TitanConstants {
    ...
    public String getUri() {
        return "/plugins/titan/freeverify/add";
    }
    ...
    public PluginResult postHandle(HttpServletRequest request) {
        ...
        Transaction t = Cat.newTransaction("TitanQconfigPlugin", "FreeVerifyAddHandler");
        try {
            ...
            if (body != null) {
                ...
                if (freeVerifyInputEntity != null) {
                    ...
                    if (sitePermission) {
                        ...
                        t.addData(debugInfo);
                        Cat.logEvent("FreeVerifyAddHandler", "SUCCESS", Event.SUCCESS, debugInfo);

                        // batch save
                        QconfigServiceUtils.batchSave(getQconfigService(), "FreeVerifyAddHandler", updateCdList, true);
                    } else {
                        t.addData("postHandleDetail(): sitePermission=false, not allow to add!");
                        Cat.logEvent("FreeVerifyAddHandler", "NO_PERMISSION", Event.SUCCESS, "sitePermission=false, not allow to add! clientIp=" + clientIp);
                        pluginResult = new PluginResult(PluginStatusCode.TITAN_KEY_CANNOT_WRITE, "Access ip whitelist check fail! clientIp=" + clientIp);
                    }

                } else {
                    t.addData("postHandleDetail(): freeVerifyInputEntity=null, no need to add!");
                }
            } else {
                t.addData("postHandleDetail(): freeVerifyInputEntity body is null or empty, no need to add!");
            }
    ...
    private ConfigDetail buildUpdateConfigDetail(String dataId, String profile, FreeVerifyInputEntity freeVerifyInputEntity) throws Exception {
        ...
        List<ConfigDetail> configDetailList = QconfigServiceUtils.currentConfigWithoutPriority(getQconfigService(), "FreeVerifyAddHandler", configFieldList);
    ...
    // merge
    private void mergeFreeVerifyField(Properties prop, String fieldName, String inputAppend) {
        if (!Strings.isNullOrEmpty(inputAppend)) {
            String oldValue = prop.getProperty(fieldName);
            if (oldValue == null) {
                oldValue = "";
            }
            Splitter splitter = Splitter.on(",").omitEmptyStrings().trimResults();
            List<String> itemList = splitter.splitToList(inputAppend);
            String newValue = CommonHelper.appendWithComma(oldValue, itemList);
