...
public class DaoOfLoginUser {
    ...
    public LoginUser getUserById(int userId) {
        try {
            return jdbcTemplate.queryForObject(
                    "SELECT id, user_no, user_name, user_email, password FROM login_users WHERE id = ?",
    ...
    public List<LoginUser> getUserByGroupId(int groupId) {
        String sql =
                "SELECT tb2.id, tb2.user_no, tb2.user_name, tb2.user_email, tb2.password, tb1.role, tb1.adduser FROM user_group tb1 "
    ...
    public int updateUserPassword(LoginUser user) {
        if (user == null)
            return -1;
        try {
            return jdbcTemplate.update("UPDATE login_users SET password=? WHERE id=?", user.getPassword(),
                    user.getId());
        } catch (Throwable e) {
            throw e;
        }
