...
public class DalGroupMemberResource {
	...
	@Path("groupuser")
	@Produces(MediaType.APPLICATION_JSON)
	public List<LoginUser> getGroupUsers(
			@QueryParam("groupId") int currentGroupId) {
		List<LoginUser> users = SpringBeanGetter.getDaoOfLoginUser()
				.getUserByGroupId(currentGroupId);
		...
		while (gite.hasNext()) {
			...
			DalGroup group = SpringBeanGetter.getDaoOfDalGroup()
					.getDalGroupById(relation.getChild_group_id());
	...
	public Status addUser(@FormParam("groupId") int currentGroupId,
		...
		if (null == userNo) {
			log.error(String
					.format("Add member failed, caused by illegal parameters:userNo=%s",
							userNo));
		...
		LoginUser user = SpringBeanGetter.getDaoOfLoginUser().getUserById(
				userID);

		List<UserGroup> ugGroups = SpringBeanGetter.getDalUserGroupDao()
				.getUserGroupByUserId(user.getId());
		...
		int ret = SpringBeanGetter.getDalUserGroupDao().insertUserGroup(userID,
				currentGroupId, user_role, adduser);
		if (ret <= 0) {
	...
	public Status update(@FormParam("groupId") int currentGroupId,
		...
		int adduser = allowAddUser == true ? 1 : 2;
		int ret = SpringBeanGetter.getDalUserGroupDao().updateUserPersimion(
				userID, currentGroupId, user_role, adduser);
		if (ret <= 0) {
	...
	public Status addGroup(@FormParam("currentGroupId") int currentGroupId,
		...
		if (null == userNo) {
			log.error(String
					.format("Add group failed, caused by illegal parameters:[userNo=%s]",
							userNo));
			Status status = Status.ERROR;
			status.setInfo("Illegal parameters.");
			return status;
		...
		GroupRelation relation = SpringBeanGetter.getGroupRelationDao()
				.getGroupRelationByCurrentGroupIdAndChildGroupId(
						currentGroupId, childGroupId);
		if (relation != null) {
			DalGroup dalGroup = SpringBeanGetter.getDaoOfDalGroup()
					.getDalGroupById(childGroupId);
			Status status = Status.ERROR;
			status.setInfo("DAL Team[" + dalGroup.getGroup_name()
					+ "]已经加入当前DAL Team.");
			return status;
		}
		...
		int ret = SpringBeanGetter.getGroupRelationDao().insertChildGroup(
				relation);
		if (ret <= 0) {
	...
	public Status updateGroup(@FormParam("currentGroupId") int currentGroupId,
		...
		int adduser = allowGroupAddUser == true ? 1 : 2;
		String updateUserNo = SpringBeanGetter.getDaoOfLoginUser()
				.getUserByNo(userNo).getUserName();
		int ret = SpringBeanGetter.getGroupRelationDao().updateGroupRelation(
				currentGroupId, childGroupId, childGroupRole, adduser,
				updateUserNo, new Timestamp(System.currentTimeMillis()));
		if (ret <= 0) {
	...
	public Status delete(@FormParam("groupId") int currentGroupId,
		...
		if (isDalTeam) {
			...
			int ret = SpringBeanGetter.getGroupRelationDao()
					.deleteChildGroupByCurrentGroupIdAndChildGroupId(
							currentGroupId, childGroupId);
			if (ret <= 0) {
		...
		int ret = SpringBeanGetter.getDalUserGroupDao().deleteUserFromGroup(
	...
	@Path("approveuser")
	@Produces(MediaType.APPLICATION_JSON)
	public List<LoginUser> getApproveUsers(
		...
		DalGroup dalGroup = SpringBeanGetter.getDaoOfDalGroup()
				.getDalGroupById(prj.getDal_group_id());
		if (dalGroup == null) {
			return null;
		}
		List<LoginUser> users = SpringBeanGetter.getDaoOfLoginUser()
				.getUserByGroupId(dalGroup.getId());
	...
	private boolean validatePermision(String userNo, int currentGroupId) {
		...
		havePermision = validateUserPermisionInCurrentGroup(userNo,
				currentGroupId);
		if (havePermision) {
			return havePermision;
		}
		havePermision = validateUserPermisionInChildGroup(userNo,
				currentGroupId);
		return havePermision;
	}

	private boolean validateUserPermisionInCurrentGroup(String userNo,
			int currentGroupId) {
		LoginUser user = SpringBeanGetter.getDaoOfLoginUser().getUserByNo(
				userNo);
		// 用户加入的所有组
	...
	private boolean validateUserPermisionInChildGroup(String userNo,
			int currentGroupId) {
		boolean havePermison = false;
		int userId = SpringBeanGetter.getDaoOfLoginUser().getUserByNo(userNo)
				.getId();
		List<GroupRelation> relations = SpringBeanGetter.getGroupRelationDao()
				.getAllGroupRelationByCurrentGroupId(currentGroupId);
			...
			if (relation.getAdduser() == 1) { // the child group can manage the
				...
				List<UserGroup> ugs = SpringBeanGetter.getDalUserGroupDao()
						.getUserGroupByGroupIdAndUserId(
								relation.getChild_group_id(), userId);
	...
	private boolean validateUserPermisionInCurrentGroup(String userNo,
			int currentGroupId, int user_role) {
		LoginUser user = SpringBeanGetter.getDaoOfLoginUser().getUserByNo(
				userNo);
		// 用户加入的所有组
		List<UserGroup> urgroups = SpringBeanGetter.getDalUserGroupDao()
		...
		for (UserGroup ug : urgroups) {
			if (ug.getGroup_id() == DalGroupResource.SUPER_GROUP_ID
					&& ug.getAdduser() == 1 && ug.getRole() <= user_role) {
				return true;
			}
			if (ug.getGroup_id() == currentGroupId && ug.getAdduser() == 1
	...
	private void transferProjectToGroup(String userNo, int groupId) {
		...
		for (UserProject proj : userProjects) {
			...
			UserProject project = SpringBeanGetter.getDaoOfUserProject()
					.getMinUserProjectByProjectId(project_id);
			// 验证当前project是否是由当前user创建
			if (proj.getId() == project.getId()) {
				// 更新Project表的groupId为当前用户的gourpId
				SpringBeanGetter.getDaoOfProject().updateProjectGroupById(
						groupId, project_id);
				// 删除user_project表中所有project_id符合当前迭代的Project
				SpringBeanGetter.getDaoOfUserProject().deleteUserProject(
						project_id);
			}
