...
#foreach($p in $method.getParameters())
				...
				query.$!{first}Constrain(p => p.${p.getName()}).Equal(${p.getAlias()});
#end	
#if(${p.getConditionType()} == "NotEqual" ) 
				query.$!{first}Constrain(p => p.${p.getName()}).NotEqual(${p.getAlias()});
#end
#if(${p.getConditionType()} == "Great" )
				query.$!{first}Constrain(p => p.${p.getName()}).Greater(${p.getAlias()});
#end
#if(${p.getConditionType()} == "lessThan" ) 
				query.$!{first}Constrain(p => p.${p.getName()}).GreaterEqual(${p.getAlias()});
#end
#if(${p.getConditionType()} == "GreatAndEqual" ) 
				query.$!{first}Constrain(p => p.${p.getName()}).Less(${p.getAlias()});
#end
#if(${p.getConditionType()} == "LessAndEqual" ) 
				query.$!{first}Constrain(p => p.${p.getName()}).LessEqual(${p.getAlias()});
#end
#if(${p.getConditionType()} == "Like" ) 
				query.$!{first}Constrain(p => p.${p.getName()}).Like(${p.getAlias()});
#end
#if(${p.getConditionType()} == "In" ) 
				query.$!{first}Constrain(p => p.${p.getName()}).In(${p.getAlias()});
#end
