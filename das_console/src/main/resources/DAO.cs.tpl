...
public #if( $method.getAction() == "select" )IDataReader#{else}int#end ${method.getMethodName()}#[[(]]##foreach($p in $parameters)${p.getType()} ${p.getName()}#if($foreach.count != $parameters.size()), #end#end#[[)]]# {
        {
            try
            {
                ...
                return this.#if( $method.getAction() == "select" )Fetch#{else}Execute#end("${method.getSqlSPName()}", parameters.ToArray());
            }
            catch (Exception ex)
            {
                throw ex;
...
public int ${method.getMethodName()}#[[(]]##foreach($p in $parameters)${p.getType()} ${p.getName()}#if($foreach.count != $parameters.size()), #end#end#[[)]]# {
        {
            try
            {
                ...
                return this.ExecuteSp("${method.getSqlSPName()}", parameters.ToArray());
            }
            catch (Exception ex)
            {
                throw ex;
