...
public class DalTableDaoShardByDbMySqlTest {
...
//			DalClientFactory.initClientFactory("/DalMult.config");
	...
	public void testCrossShardInsert() {
		try {
			...
			res = dao.crossShardCombinedInsert(new DalHints(), keyHolders, pList);
	...
	public void testCrossShardBatchInsert() {
		try {
			...
			Assert.assertArrayEquals(new int[]{1, 1, 1}, res.get("0"));
			Assert.assertArrayEquals(new int[]{1, 1, 1}, res.get("1"));
		} catch (Exception e) {
			e.printStackTrace();
			Assert.fail();
		}
