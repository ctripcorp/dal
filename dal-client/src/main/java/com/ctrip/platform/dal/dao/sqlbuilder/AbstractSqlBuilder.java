...
public abstract class AbstractSqlBuilder implements SqlBuilder {
     ...
     * 
     * It will append where and check if the value is start of "and" or "or", of so, the leading "and" or "or" will be
     * removed.
     */
    @Override
    public String build() {
    ...
    protected String buildWithTableShardString(String tableShardStr) {
        ...
        try {
            List<Clause> clauseList = clauses.getList();
            clauseList = meltdown(clauseList);
            return concat(clauseList, tableShardStr);
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
    }
    ...
    /**
     * If there is COMMA, then the leading space will not be appended. If there is bracket, then both leading and
    ...
    private String concat(List<Clause> clauseList, String tableShardStr) throws SQLException {
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < clauseList.size(); i++) {
            Clause curClause = clauseList.get(i);
            Clause nextClause = (i == clauseList.size() - 1) ? null : clauseList.get(i + 1);

            if (curClause instanceof AbstractFreeSqlBuilder.Table) {
                AbstractFreeSqlBuilder.Table table = (AbstractFreeSqlBuilder.Table) curClause;
                sb.append(table.build(tableShardStr));
            } else {
                sb.append(curClause.build());
            }
    ...
    /**
     * Builder will not insert space if enableSmartSpaceSkipping is enabled and: 1. current cuase is operator(AND, OR,
     * NOT) 2. current cuase is left bracket 3. next clause is right bracket or COMMA
     */
