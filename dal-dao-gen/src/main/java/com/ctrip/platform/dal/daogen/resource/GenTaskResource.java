...
public class GenTaskResource {
    ...
    public TaskAggeragation getTasks(@QueryParam("project_id") int id) {
        ...
        List<GenTaskByTableViewSp> tableViewSpTasks = SpringBeanGetter.getDaoByTableViewSp().getTasksByProjectId(id);
    ...
    @Path("sql_class")
    @Produces(MediaType.APPLICATION_JSON)
    public FreeSqlClassPojoNames getClassPojoNames(@QueryParam("project_id") int id, @QueryParam("db_name") String db_name) {
        List<GenTaskByFreeSql> sqlTasks = SpringBeanGetter.getDaoByFreeSql().getTasksByProjectId(Integer.valueOf(id));
    ...
    private ApproveTask haveApprovePermision(int approverId, int taskId, String taskType) {
    ...
    public List<ApproveTask> getMyApproveTask(@Context HttpServletRequest request, @QueryParam("rand") String rand) {
        ...
        while (ite.hasNext()) {
            ApproveTask task = ite.next();
            String create_user_name = SpringBeanGetter.getDaoOfLoginUser().getUserById(task.getCreate_user_id()).getUserName();
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public List<ApproveTaskDetail> getMyApproveTaskDetail(@QueryParam("rand") String rand, @QueryParam("taskId") String taskId, @QueryParam("taskType") String taskType) {
    ...
    private List<ApproveTaskDetail> buildStandardTaskDetail(GenTaskByTableViewSp task) {
        ...
        detail.add(createOneAttr("Have Pagination Fun:", String.valueOf(task.isPagination())));
        ...
        if (task.getApi_list() != null && !task.getApi_list().isEmpty()) {
            ...
            for (int i = 0; i < apis.length; i++) {
                try {
                    ...
                    funs.append(api.getMethod_declaration()).append("<br/><br/>");
                } catch (Exception e) {
                }
            }
        }
