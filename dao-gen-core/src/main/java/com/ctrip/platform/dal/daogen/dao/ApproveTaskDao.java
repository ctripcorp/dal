...
public class ApproveTaskDao {
    ...
    public List<ApproveTask> getAllApproveTaskByApproverId(Integer approverId) {
        return this.jdbcTemplate.query("SELECT id, task_id, task_type, create_time, create_user_id, approve_user_id FROM approve_task WHERE approve_user_id=?",
    ...
    public int insertApproveTask(ApproveTask task) {
        return this.jdbcTemplate.update("INSERT INTO approve_task (task_id, task_type, create_time, create_user_id, approve_user_id) VALUE(?,?,?,?,?)",
                task.getTask_id(), task.getTask_type(), task.getCreate_time(), task.getCreate_user_id(), task.getApprove_user_id());
    }

    public int deleteApproveTaskById(Integer id) {
        try {
            return this.jdbcTemplate.update("DELETE FROM approve_task WHERE id=?", id);
