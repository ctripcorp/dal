...
(function(window, undefined) {
	...
	var post_task_table_view_sp = function(postData) {
		postData["table_names"] = $('#table_list').multipleSelect('getSelects')
				.join(",");
		postData["view_names"] = $('#view_list').multipleSelect('getSelects')
				.join(",");
	...
	var post_task_auto = function(postData) {
		...
		if ("select" == postData["crud_type"]) {
			...
			postData["orderby"] = sprintf("%s,%s", $("#orderby_field").val(),
					$("#orderby_sort").val());
		} else {
			postData["scalarType"] = "";
		...
		if ($("#crud_option").val() == "insert") {
			...
			$.each($("#param_list_auto").children("div"),
					function(index, value) {
						...
						selectedConditions.push(sprintf("%s,%s,%s,%s,%s",
								paramValues[index], 1, paramValues[index],
	...
	var post_task_free_sql = function(postData) {
		...
		if ($("#free_sql_crud_option").val() == "select") {
			...
			if (postData["class_name"] == "" || postData["pojo_name"] == ""
		...
		$.each($("#param_list").children("div"),
				function(index, value) {
					...
					if (postData["sql_style"] == "csharp") {
						...
						var third = $(value).children("div").eq(0).children(
								":checkbox").eq(0);
						paramList.push(sprintf("%s,%s,%s", $(first).val(), $(
								second).val(), $(third).is(":checked")));
					}
				});
		postData["params"] = paramList.join(";");
	...
	AjaxUtil.prototype = {
		...
		reload_dbservers : function(callback, groupDBs, groupId) {
			...
			$.get(url).done(
					function(data) {
						...
						$("#databases")[0].selectize.on('dropdown_open',
								function(dropdown) {
									$(".step1").height(240);
								});

						$("#databases")[0].selectize.on('dropdown_close',
								function(dropdown) {
									$(".step1").height(74);
								});
		...
		reload_dbsets : function(callback) {
			var selectedProject = $.jstree.reference("#jstree_projects")
					.get_selected();
			...
			var project = $.jstree.reference("#jstree_projects").get_node(
					selectedProject[0]).original;
			cblock($("body"));
			$.get(
					"/rest/groupdbset/getDbset?rand=" + Math.random()
		...
		generate_code : function() {
			...
			$
					.post(
							...
							function(data) {
								if (data.code != "OK") {
									$("#generateCodeProcessErrorMess").html(
											data.info);
									$("#generateCodeProcessErrorDiv").modal();
									progress
											.reportException("generate success return but not ok");
								}
							}).fail(function(data) {
