(function(window, undefined) {
	...
	var loadConfigType = function() {
		$.get("/rest/configTemplate/getConfigList", {
			rand : Math.random()
		}).done(
				function(data) {
					if ($("#configType")[0] != undefined
							&& $("#configType")[0].selectize != undefined) {
	...
	var loadLanguageType = function() {
		$.get("/rest/configTemplate/getLanguageList", {
			rand : Math.random()
		}).done(
				function(data) {
					if ($("#languageType")[0] != undefined
							&& $("#languageType")[0].selectize != undefined) {
	...
	var showDalConfigDemo = function() {
		...
		$("#dalConfigDemoModal div[class='modal-dialog']").css({
			'min-width' : imgW + 40 + 'px'
		});
	};
	...
	var refreshDbSet = function() {
		...
		cblock($("body"));
		$.get(
				"/rest/groupdbset/getDbset?groupId=" + current_group + "&rand="
	...
	var refreshDbSetEntry = function() {
		...
		cblock($("body"));
		$.get(
				"/rest/groupdbset/getDbsetEntry?dbsetId=" + record['id']
	...
	var editDbSetEntry = function() {
		...
		$.get(
				...
				function(data) {
					...
					$("#databases2")[0].selectize
							.setValue(record['connectionString']);
					...
					.jstree(
							{
								'core' : {
									...
									'data' : {
										'url' : function(node) {
											return node.id == "#" ? "/rest/groupdbset?root=true&rand="
													+ Math.random()
													: "/rest/groupdbset?rand="
															+ Math.random();
										}
									}
								}
			...
			.ready(
					function() {
								...
								.on(
										...
										function(event) {
											...
											var connectionString = $(
													"#databases").val();
											if (dbsetentryname == null
													|| dbsetentryname == "") {
												$("#adddbsetentry_error_msg")
														.html(
																'databaseSet Entry name 不能为空!');
												return;
											}
						...
						$(document.body)
								.on(
										'click',
										'#save_updatedbsetentry',
										function(event) {
											$("#updatedbsetentry_error_msg")
													.html('');
											var dbsetentryname = $(
													"#dbsetentryname2").val();
											var databaseType = $(
													"#databaseType2").val();
											var sharding = $("#sharding2")
													.val();
											var connectionString = $(
													"#databases2").val();
											if (dbsetentryname == null
											...
											var records = w2ui['previewgrid']
													.getSelection();
											var record = w2ui['previewgrid']
													.get(records[0]);
											$
													.post(
															"/rest/groupdbset/updateDbsetEntry",
															{
																"id" : record['id'],
																"name" : dbsetentryname,
																"databaseType" : databaseType,
						...
						$("#save_configtemplate").click(function() {
