...
/**
 * This handler is to add whiteList to 'permission' field for titan key.
 * <p>
 * Sample:
 * [POST]   http://qconfig.fat16.qa.nt.ctripcorp.com/plugins/titan/whitelist/add?
 * &env=pro
 * &env=pro,uat,fat
 * <p>
 * [body] sample as bellow:
 * titanKey=siccdb_w,siccdb_r&clientAppId=100001681,100001680&runInBigData=false
 * <p>
 * <p>
 * 说明:
 ...
 * <p>
 * Created by lzyan on 2018/09/25, 2018/11/12.
 */
public class WhiteListAddHandler extends BaseAdminHandler implements TitanConstants {
    private static Logger logger = LoggerFactory.getLogger(WhiteListAddHandler.class);

    public WhiteListAddHandler(QconfigService qconfigService, PluginConfigManager pluginConfigManager) {
        super(qconfigService,pluginConfigManager);
    }

    @Override
    public String getUri() {
        return "/plugins/titan/whitelist/add";
    }
    ...
    public PluginResult preHandle(HttpServletRequest request) {
        ...
        //format <runInBigData>
        String runInBigData = request.getParameter("runInBigData");
        request.setAttribute(REQ_ATTR_TITAN_RUN_IN_BIGDATA, runInBigData);
    ...
    public PluginResult postHandle(HttpServletRequest request) {
        ...
        Transaction t = Cat.newTransaction("TitanQconfigPlugin", "WhiteListAddHandler");
        try {
            ...
            if (sitePermission) {
                ...
                // get 'runInBigData'
                boolean runInBigData = false;
                String ribd = (String) request.getAttribute(REQ_ATTR_TITAN_RUN_IN_BIGDATA);
                if (!Strings.isNullOrEmpty(ribd)) {
                    runInBigData = Boolean.parseBoolean(ribd);
                }
                ...
                for (String titanKey : titanKeyList) {
                    ...
                    List<ConfigDetail> cdList = buildCdListPerKey(request, titanKey, subEnvList, runInBigData);
                ...
                t.addData(debugInfo);
                Cat.logEvent("WhiteListAddHandler", "SUCCESS", Event.SUCCESS, debugInfo);

                // batch save
                if (updateCdList != null && !updateCdList.isEmpty()) {
                    QconfigServiceUtils.batchSave(getQconfigService(), "WhiteListAddHandler", updateCdList, true);
                }
            } else {
                t.addData("postHandleDetail(): sitePermission=false, not allow to add!");
                Cat.logEvent("WhiteListAddHandler", "NO_PERMISSION", Event.SUCCESS, "sitePermission=false, not allow to add! clientIp=" + clientIp);
    ...
    private List<ConfigDetail> buildCdListPerKey(HttpServletRequest request, String titanKey, List<String> subEnvList, boolean runInBigData) throws Exception {
        ...
        for (String env : envList) {
            ...
            for (String suffix : suffixList) {
                ...
                for (String subEnv : subEnvList) {
                    ...
                    cd = buildUpdateConfigDetail(request, finalKeyName, profile, runInBigData);
                    if (cd != null) {
                        cdList.add(cd);
                    }
                }
    ...
    private ConfigDetail buildUpdateConfigDetail(HttpServletRequest request, String dataId, String profile, boolean runInBigData) throws Exception {
        ConfigDetail cd = null;
        ...
        List<ConfigDetail> configDetailList = QconfigServiceUtils.currentConfigWithoutPriority(getQconfigService(), "WhiteListAddHandler", configFieldList);
        if (configDetailList != null && !configDetailList.isEmpty()) {
            ...
            String permissionsNew = CommonHelper.appendWithComma(permissionsOld, clientAppIdList);
            ...
            boolean freeVerifyAppIdIdempotent = true;
            if (runInBigData) {
                // 添加 clientAppId 到 freeVerifyAppIdList
                String freeVerifyAppIdListOld = prop.getProperty(FREE_VERIFY_APPID_LIST);
                if (freeVerifyAppIdListOld == null) {
                    freeVerifyAppIdListOld = "";
                }
                String freeVerifyAppIdListNew = CommonHelper.appendWithComma(freeVerifyAppIdListOld, clientAppIdList);
                prop.put(FREE_VERIFY_APPID_LIST, freeVerifyAppIdListNew);
                freeVerifyAppIdIdempotent = CommonHelper.idempotentCheck(freeVerifyAppIdListNew, freeVerifyAppIdListOld);
            }

            // 如果 permissionIdempotent, freeVerifyAppIdIdempotent 这2个幂等都为true, 则返回null(不save)
            if (permissionIdempotent && freeVerifyAppIdIdempotent) {
                ...
                Cat.logEvent("TitanPlugin.WhiteListAdd.Idempotent.Ignore.Save", eventName, Event.SUCCESS, clientAppId);
