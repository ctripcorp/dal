...
public class NewHintsOfCodeGenOnMysqlDaoUnitTest {
	...
	public void setUp() throws Exception {
		...
		List<NewHintsOfCodeGenOnMySql> daoPojos1 = new ArrayList<NewHintsOfCodeGenOnMySql>(
				3);

		for (int i = 0; i < 3; i++) {
			NewHintsOfCodeGenOnMySql daoPojo = new NewHintsOfCodeGenOnMySql();
			daoPojo.setAge(i + 20);
		...
		List<NewHintsOfCodeGenOnMySql> daoPojos2 = new ArrayList<NewHintsOfCodeGenOnMySql>(
				3);
		for (int i = 0; i < 3; i++) {
			NewHintsOfCodeGenOnMySql daoPojo = new NewHintsOfCodeGenOnMySql();
			daoPojo.setAge(i + 20);
...
//		DalHintsChecker.checkNull(hints,notNullItems);
	...
	public void testtest_build_query_allShard_async() throws Exception {
		...
		Integer Age = 20;// Test value here
		DalHints hints = new DalHints();
		List<NewHintsOfCodeGenOnMySql> ret = dao
				.test_build_query_allShard_async(Age, hints);
		assertNull(ret);
		Future<List<NewHintsOfCodeGenOnMySql>> fr = (Future<List<NewHintsOfCodeGenOnMySql>>) hints
				.getAsyncResult();
		ret = fr.get(); // 异步返回结果
...
//		assertEquals("Initial_Shard_00", ret.get(1).getName());
	...
	public void testtest_build_query_shards_callback() throws Exception {
		...
		callback.waitForDone();
		ret = (List<NewHintsOfCodeGenOnMySql>) callback.getResult(); // 异步返回结果
