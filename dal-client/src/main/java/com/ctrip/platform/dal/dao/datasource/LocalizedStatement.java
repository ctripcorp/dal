...
import static com.ctrip.platform.dal.dao.datasource.log.OperationType.*;
...
public class LocalizedStatement implements Statement, LocalizationValidatable {
    ...
    protected void validateLocalization(OperationType operationType) throws DalException {
        if (!validate(operationType))
            throw new DalException(ErrorCode.NonLocalRequestBlocked,
                    "Non-local operation is not allowed for drc cluster");
    }

    @Override
    public boolean validate(OperationType operationType) {
        if (validator == null) {
            lastValidationStatus = ValidationStatus.SKIPPED;
            lastValidationResult = new ValidationResult(true, null, null);
            return true;
        }
        ValidationResult result = validator.validateRequest(operationType);
    ...
    public ResultSet executeQuery(String sql) throws SQLException {
        validateLocalization(QUERY);
        return statement.executeQuery(sql);
    }

    @Override
    public int executeUpdate(String sql) throws SQLException {
        validateLocalization(getOperationType(sql));
    ...
    protected OperationType diffInsertFromUpdate(char firstAlphaCharUc) {
        return SqlUtils.isInsertOperation(firstAlphaCharUc) ? INSERT : UPDATE;
    }
