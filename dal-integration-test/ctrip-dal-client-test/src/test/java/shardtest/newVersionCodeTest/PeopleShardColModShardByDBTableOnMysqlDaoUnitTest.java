...
public class PeopleShardColModShardByDBTableOnMysqlDaoUnitTest {
    ...
    private static final String SIMPLE_SHARD_DATA_BASE = "SimpleShardByDBTableOnMysql";
    private static DalClient client = null;
    private static PeopleShardColModShardByDBTableOnMysqlDao dao = null;
    private static PeopleShardColModShardByDBTableOnMysqlDao simple_shard_dao = null;
    ...
    public static void setUpBeforeClass() throws Exception {
        ...
        simple_shard_dao = new PeopleShardColModShardByDBTableOnMysqlDao(SIMPLE_SHARD_DATA_BASE);
    }
...
//		dao.test_def_truncate(new DalHints().inShard(1),"_1");
    ...
    @Test
    public void testCountInSimpleShard() throws Exception {
       try{
           simple_shard_dao.count(new DalHints().inAllShards());
           Assert.fail();
       }catch (Exception e){
           assertTrue(e.getMessage().contains("Can't locate all table shards"));
       }

        try{
            simple_shard_dao.count(new DalHints().inAllShards().inAllTableShards());
            Assert.fail();
        }catch (Exception e){
           assertTrue(e.getMessage().contains("Can't locate all table shards"));
        }
    ...
    @Test
    public void testCountCrossTableShards() throws Exception {
        int affected = dao.count(new DalHints().inShard(0).inAllTableShards());
        assertEquals(6, affected);

        Set<String> tableShards = new HashSet<>();
        tableShards.add("1");

        affected = dao.count(new DalHints().inShard(1).inTableShards(tableShards));
        assertEquals(3, affected);
        ...
        affected = dao.count(new DalHints().inAllShards());
        assertEquals(12, affected);
    }
    ...
    @Test
    public void testQueryByCrossTableShards() throws Exception {
        MysqlPeopleTable pojo = new MysqlPeopleTable();
        pojo.setAge(20);

        List<MysqlPeopleTable> ret = dao.queryBy(pojo, new DalHints().inAllShards());
        assertEquals(2, ret.size());

        ret = dao.queryBy(pojo, new DalHints().inAllShards().inAllTableShards());
        assertEquals(2, ret.size());

        Set<String> tableShards = new HashSet<>();
        tableShards.add("1");

        ret = dao.queryBy(pojo, new DalHints().inAllShards().inTableShards(tableShards));
        assertEquals(0, ret.size());
    }

    @Test
    public void testQueryAllCrossTableShards() throws Exception {
        List<MysqlPeopleTable> list = dao.queryAll(new DalHints().inShard(0).inAllTableShards());
        assertEquals(6, list.size());

        Set<String> tableShards = new HashSet<>();
        tableShards.add("1");

        list = dao.queryAll(new DalHints().inShard(0).inTableShards(tableShards));
        assertEquals(3, list.size());
        ...
        list = dao.queryAll(new DalHints().inAllShards());
        assertEquals(12, list.size());
    }
    ...
    public void testtest_build_delete() throws Exception {
        ...
        assertEquals(1, ret);
        List<DalHintEnum> notNullItems = new ArrayList<DalHintEnum>() {{
            add(DalHintEnum.shardColValues);
            add(DalHintEnum.tableShard);
        }};
        DalHintsChecker.checkEquals(original, hints, notNullItems);
    ...
    @Test
    public void testtest_build_delete_crossTableShards() throws Exception {
        List<Integer> CityID = new ArrayList<>(2);
        CityID.add(200);
        CityID.add(201);
        ...
        int ret = dao.test_build_delete(CityID, Age, new DalHints().inAllShards().inAllTableShards());
        assertEquals(4, ret);
        ...
        ret = dao.test_build_delete(CityID, Age, new DalHints().shardBy("CityID").tableShardBy("Age"));
        assertEquals(4, ret);
    ...
    @Test
    public void testtest_build_insert_crossTableShards() throws Exception {
        String Name = "test_build_insert_crossTableShards";// Test value here
        Integer CityID = 206;// Test value here
        Integer Age = 26;// Test value here
        int ret = dao.test_build_insert(Name, CityID, Age, new DalHints().inAllShards().inAllTableShards());
        assertEquals(4, ret);

        int affected = dao.count(new DalHints().inAllShards());
        assertEquals(16, affected);

        ret = dao.test_build_insert(Name, CityID, Age, new DalHints().inShard(0).inAllTableShards());
        assertEquals(2, ret);

        affected = dao.count(new DalHints().inAllShards());
        assertEquals(18, affected);
    }
    ...
    @Test
    public void testtest_build_update_crossTableShards() throws Exception {
        String Name = "test_build_update_crossTableShards";
        ...
        int ret = dao.test_build_update(Name, CityID, Age, new DalHints().inAllShards().inAllTableShards());
        assertEquals(4, ret);

        MysqlPeopleTable pojo = dao.queryByPk(1, new DalHints().inShard(0).inTableShard(0));
        assertEquals("test_build_update_crossTableShards", pojo.getName());
        ...
        Name = "test_build_update_tableShardBy";

        dao.test_build_update(Name, CityID, Age, new DalHints().shardBy("CityID").tableShardBy("Age"));
        assertEquals(4, ret);

        pojo = dao.queryByPk(1, new DalHints().inShard(0).inTableShard(0));
        assertEquals("test_build_update_tableShardBy", pojo.getName());
    ...
    @Test
    public void testtest_build_query_listFirst_crossTableShards() throws Exception {
        List<Integer> CityID = new ArrayList<>(2);
        CityID.add(200);
        CityID.add(201);
        ...
        MysqlPeopleTable ret = dao.test_build_query_listFirst(CityID, Age, new DalHints().shardBy("CityID").tableShardBy("Age"));
        assertEquals("InsertByfields_0fields_0", ret.getName());
    }
...
//			System.out.println("4: "+ret.get(i).getName());
    ...
    @Test
    public void testtest_build_query_list_crossTableShards() throws Exception {
        List<Integer> CityID = new ArrayList<>(2);
        CityID.add(200);
        CityID.add(201);
        ...
        List<MysqlPeopleTable> ret = dao.test_build_query_list(CityID, Age, new DalHints().shardBy("CityID").tableShardBy("Age"));
        assertEquals(8, ret.size());

        ret = dao.test_build_query_list(CityID, Age, new DalHints().inAllShards().inAllTableShards());
        assertEquals(8, ret.size());

        ret = dao.test_build_query_list(CityID, Age, new DalHints().inAllShards().inAllTableShards().inTableShard(0));
        assertEquals(8, ret.size());

        Set<String> tableShards = new HashSet<>();
        tableShards.add("0");
        ret = dao.test_build_query_list(CityID, Age, new DalHints().inAllShards().inTableShards(tableShards));
        assertEquals(4, ret.size());

        ret = dao.test_build_query_list(CityID, Age, new DalHints().inShard(0).inTableShards(tableShards));
        assertEquals(2, ret.size());
    }

    @Test
    public void testtest_build_query_list_crossTableShards_priority() throws Exception {
        List<Integer> CityID = new ArrayList<>(2);
        CityID.add(200);
        CityID.add(201);
        ...
        List<MysqlPeopleTable> ret = dao.test_build_query_list(CityID, Age, new DalHints().inAllShards().inAllTableShards().inTableShard(0));
        assertEquals(4, ret.size());

        dao.test_build_query_list(CityID, Age, new DalHints().inAllShards().inAllTableShards().tableShardBy("Age"));
        assertEquals(4, ret.size());

        dao.test_build_query_list(CityID, Age, new DalHints().inAllShards().inTableShard(1).tableShardBy("Age"));
        assertEquals(4, ret.size());

        Set<String> tableShards = new HashSet<>();
        tableShards.add("1");
        ret = dao.test_build_query_list(CityID, Age, new DalHints().inAllShards().inTableShards(tableShards).tableShardBy("Age"));
        assertEquals(0, ret.size());
...
//			System.out.println("3: "+ret.get(i).getName());
    ...
    public void testFreeSqlQueryList() throws Exception {
        ...
        List<MysqlPeopleTable> ret = dao.testFreeSqlQueryList(CityID, Age, new DalHints().setShardColValue("CityID", 200).setTableShardValue(20));
        assertEquals(2, ret.size());
    ...
    @Test
    public void testFreeSqlQueryListCrossTableShards() throws Exception {
        List<Integer> CityID = new ArrayList<>(2);
        CityID.add(200);
        CityID.add(201);
        ...
        List<MysqlPeopleTable> ret = dao.testFreeSqlQueryList(CityID, Age, new DalHints().inAllShards().inAllTableShards());
        assertEquals(8, ret.size());

        ret = dao.testFreeSqlQueryList(CityID, Age, new DalHints().shardBy("CityID").tableShardBy("Age"));
        assertEquals(8, ret.size());


        ret = dao.testFreeSqlQueryList(CityID, Age, new DalHints().inAllShards());
        assertEquals(8, ret.size());

        Set<String> tableShards=new HashSet<>();
        tableShards.add("0");
        ret = dao.testFreeSqlQueryList(CityID, Age, new DalHints().inAllShards().inTableShards(tableShards));
        assertEquals(4, ret.size());

        ret = dao.testFreeSqlQueryList(CityID, Age, new DalHints().inShard(0).inTableShards(tableShards));
        assertEquals(2, ret.size());
    }
