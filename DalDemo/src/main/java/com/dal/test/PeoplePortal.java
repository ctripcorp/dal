...
//		testSelectSP(response);
	...
	private void insertPeople(HttpServletRequest request, HttpServletResponse response) throws Exception {
		DalHints hints = readHints(request);
		int num = dao.insertPeople(getInt(request, "CityID"), get(request, "Name"), getInt(request, "ProvinceID"), getInt(request, "CountryID"), hints);
		buildResponse(num, hints, response);
	}
	
	private void deletePeople(HttpServletRequest request, HttpServletResponse response) throws Exception {
		DalHints hints = readHints(request);
		int num = dao.deletePeople(getInt(request, "CityID"), getIntList(request, "CountryID"), get(request, "Name"), hints);
		buildResponse(num, hints, response);
	}
	
	private void updatePeople(HttpServletRequest request, HttpServletResponse response) throws Exception {
		DalHints hints = readHints(request);
		int num = dao.updatePeople(get(request, "Name"), getInt(request, "CountryID"), get(request, "name"), getInt(request, "prov1"), getInt(request, "prov2"), getIntList(request, "CityIDs"), hints);
		buildResponse(num, hints, response);
	}
	...
	private List<Integer> getIntList(HttpServletRequest request, String name) {
