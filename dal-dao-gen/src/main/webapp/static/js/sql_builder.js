(function(window, undefined) {
	...
	var wrapColumn = function(value) {
		...
		if ($(".step2-2-1").attr("dbCatalog") == "MySql") {
			...
			value = sprintf("%s%s%s", SQLServerDelimiter[0], value,
					SQLServerDelimiter[1]);
		}
		return value;
	};
			...
			if (splited.length == 1) {
				...
				if (splited[1] == "6") {// between
					if ($("#sql_style").val() == "csharp") {
						conditions
								.push(sprintf(
										" %s BETWEEN @%s_start AND @%s_end ",
										wrapColumn(splited[0]), splited[0],
										splited[0]));
					} else {
						conditions.push(sprintf(" %s BETWEEN ? AND ? ",
								wrapColumn(splited[0])));
					}
				} else if (splited[1] == "8") { // in
					if ($("#sql_style").val() == "csharp") {
						conditions.push(sprintf(" %s in (@%s) ",
								wrapColumn(splited[0]), splited[0]));
					} else {
						conditions.push(sprintf(" %s in (?) ",
								wrapColumn(splited[0])));
					}
					...
					// not
					// null
					conditions.push(sprintf(" %s %s ", wrapColumn(splited[0]),
							whereCondition[splited[1]]));
				} else {
					if ($("#sql_style").val() == "csharp") {
						conditions.push(sprintf(" %s %s @%s ",
								wrapColumn(splited[0]),
								whereCondition[splited[1]], splited[0]));
					} else {
						conditions.push(sprintf(" %s %s ? ",
								wrapColumn(splited[0]),
								whereCondition[splited[1]]));
					}
				}
	...
	var buildSelectSQL = function(conditions) {
		...
		if (conditions != undefined && conditions.length > 0) {
			select_sql_builder = sprintf("SELECT %s FROM %s WHERE %s",
					wrapedFields.join(","), wrapTable($("#tables").val()),
					conditions.join(" "));
		} else {
			select_sql_builder = sprintf("SELECT %s FROM %s", wrapedFields
					.join(","), wrapTable($("#tables").val()));
		}

		if ($("#orderby_field").val() != '-1') {
			select_sql_builder = sprintf("%s ORDER BY %s %s",
					select_sql_builder, $("#orderby_field").val(), $(
							"#orderby_sort").val());
		}
	...
	var buildInsertSQL = function() {
		...
		ace.edit("sql_builder").setValue(
				sprintf("INSERT INTO %s (%s) VALUES (%s)", $("#tables").val(),
						wrapedFields.join(","), placeHodler.join(",")));
		ace.edit("sql_builder").setReadOnly(true);
	};
	...
	var buildUpdateSQL = function(conditions) {
		...
		$.each($('#fields').multipleSelect('getSelects'),
				function(index, value) {
					if ($("#sql_style").val() == "csharp") {
						placeHodler.push(sprintf(" %s = @%s ",
								wrapColumn(value), value));
					} else {
						placeHodler
								.push(sprintf(" %s = ? ", wrapColumn(value)));
					}
				});

		if (conditions.length > 0) {
			ace.edit("sql_builder").setValue(
					sprintf("UPDATE %s SET %s WHERE %s", $("#tables").val(),
							placeHodler.join(","), conditions.join(" ")));
		} else {
			ace.edit("sql_builder").setValue(
					sprintf("UPDATE %s SET %s ", $("#tables").val(),
							placeHodler.join(",")));
		}
		ace.edit("sql_builder").setReadOnly(true);
	};
	...
	var buildDeleteSQL = function(conditions) {
		if (conditions.length > 0) {
			ace.edit("sql_builder").setValue(
					sprintf("Delete FROM %s WHERE %s", $("#tables").val(),
							conditions.join(" ")));
		} else {
			ace.edit("sql_builder").setValue(
					sprintf("Delete FROM %s", $("#tables").val()));
		}
		ace.edit("sql_builder").setReadOnly(true);
	};
	...
	sql_builder.prototype = {
		...
		buildPagingSQL : function(callable) {
			...
			if ($("#gen_style").val() == "auto") { // 构建SQL（生成的代码绑定到模板）
				...
				cblock($("body"));
				$.post("/rest/task/auto/buildPagingSQL", postData,
						function(data) {
							if (data.code == "OK") {
				...
				cblock($("body"));
				$.post("/rest/task/sql/buildPagingSQL", postData,
						function(data) {
							if (data.code == "OK") {
		...
		getDatabaseCategory : function() {
			...
			$.post("/rest/task/auto/getDatabaseCategory", postData,
					function(data) {
						if (data.code == "OK") {
