 ...
 */
public class DalClusterAddHandler extends DalClusterBaseHandler implements DalConstants {

    private static final Logger LOGGER = LoggerFactory.getLogger(DalClusterAddHandler.class);

    private static final String URI = "/plugins/dal/config/add";
    private static final String METHOD = "POST";
    private static final String CAT_EVENT_TYPE = "DalPlugin.Add.Cluster";

    public DalClusterAddHandler(QconfigService qconfigService, PluginConfigManager pluginConfigManager) {
    ...
    public PluginResult postHandle(HttpServletRequest request) {
        ...
        Transaction t = Cat.newTransaction("Dal.Admin.Plugin", "DalClusterAddHandler");
        try {
            ...
            if (!Strings.isNullOrEmpty(body)) {
                ...
                if (dalClusterEntities != null && dalClusterEntities.length > 0) {
                    ...
                    if (permitted) {
                        int saveCount = add(request, dalClusterEntities, operator);
                        t.addData("postHandleDetail(): saveCount=" + saveCount);
                    } else {
                        t.addData("postHandleDetail(): sitePermission=false, not allow to add!");
                        Cat.logEvent("DalClusterAddHandler", "NO_PERMISSION", Event.SUCCESS, "sitePermission=false, not allow to add! clientIp=" + clientIp);
    ...
    private int add(HttpServletRequest request, DalClusterEntity[] addEntity, String operator) throws Exception {
        ...
        for (DalClusterEntity dalCluster : addEntity) {
            ...
            Cat.logEvent("DalClusterAddPlugin.ClusterName", dataId);
            ...
            if (configDetailList != null && !configDetailList.isEmpty()) {
                //存在，报错
                throw new DbConfigPluginException(String.format("Dal cluster[%s] 已存在.", dalCluster.getClusterName()));
            } else {
                // 新增
        ...
        if (!configDetails.isEmpty()) {
            ...
            saveCount = QconfigServiceUtils.batchSave(getQconfigService(), "DalClusterAddHandler", configDetails, true);
