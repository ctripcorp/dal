...
public class DalGroupDBDao {
	...
	public List<DalGroupDB> getAllGroupDbs() {
		return this.jdbcTemplate
				.query("select t1.id as id, t1.dbname as dbname,t2.group_name as comment,t1.dal_group_id as dal_group_id,"
	...
	public List<DalGroupDB> getGroupDBsByGroup(int groupId) {
		return this.jdbcTemplate
				.query("select id, dbname, comment,dal_group_id,"
	...
	public DalGroupDB getGroupDBByDbName(String dbname) {
		List<DalGroupDB> dbs = this.jdbcTemplate
				.query("select id, dbname, comment,dal_group_id,"
						+ "db_address, db_port, db_user, db_password, db_catalog, db_providerName from alldbs"
	...
	public List<DalGroupDB> getGroupDbsByDbNames(Set<String> dbNames) {
		try {
			return this.jdbcTemplate
					.query(String
							.format("select id, dbname, comment,dal_group_id,"
									+ "db_address, db_port, db_user, db_password, db_catalog, db_providerName from alldbs"
									+ " where dbname in (%s) ",
									StringUtils.join(dbNames, ",")),
							new RowMapper<DalGroupDB>() {
								public DalGroupDB mapRow(ResultSet rs,
	...
	public int updateGroupDB(int id, String dbname, String db_address,
			String db_port, String db_user, String db_password,
			String db_catalog, String db_providerName) {
		return this.jdbcTemplate
				.update("update alldbs set dbname=?, db_address=?, db_port=?, db_user=?, db_password=?, db_catalog=?, db_providerName=?"
						+ " where id=?", dbname, db_address, db_port, db_user,
						db_password, db_catalog, db_providerName, id);
	}

	public int updateGroupDB(int id, String comment) {
		return this.jdbcTemplate.update("update alldbs set comment=?"
				+ " where id=?", comment, id);
	}

	public int updateGroupDB(int id, Integer groupId) {
		return this.jdbcTemplate.update("update alldbs set dal_group_id=?"
				+ " where id=?", groupId, id);
	}
