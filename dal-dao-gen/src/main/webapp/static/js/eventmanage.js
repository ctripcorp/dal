(function(window, undefined) {
	...
	var refreshAllApproveDAO = function() {
		...
		cblock($("body"));
		$.get("/rest/task/getMyApproveTask?rand=" + Math.random(),
				function(data) {
					var allApproveDAO = [];
	...
	var refreshApproveTaskDetail = function() {
		...
		cblock($("body"));
		$.get(
				"/rest/task/getMyApproveTaskDetail?taskId=" + record['task_id']
						+ "&taskType=" + record['task_type'] + "&rand="
	...
	var approveOk = function() {
		...
		cblock($("body"));
		$.get(
				"/rest/task/taskApproveOperation?taskId=" + record['task_id']
						+ "&taskType=" + record['task_type'] + "&approveFlag=2"
						+ "&rand=" + Math.random(),
				function(data) {
					if (data['code'] != 'OK') {
						alert(data['info']);
						...
						alert("审批操作成功!");
						w2ui['grid_toolbar']
								.click('refreshAllApproveDAO', null);
					}
					$("body").unblock();
				}).fail(function(data) {
			alert("审批操作失败!");
						...
						$(document.body)
								.on(
										...
										function(event) {
											...
											$
													.get(
															"/rest/task/taskApproveOperation?taskId="
																	+ record['task_id']
																	+ "&taskType="
																	+ record['task_type']
																	+ "&approveFlag=3"
