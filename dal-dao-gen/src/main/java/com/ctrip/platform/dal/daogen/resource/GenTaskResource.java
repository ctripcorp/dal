...
public class GenTaskResource {
	...
	public Status checkDaoNameConflict(@FormParam("project_id") int project_id,
		...
		List<GenTaskByTableViewSp> tableViewSpTasks = SpringBeanGetter.getDaoByTableViewSp()
				.getTasksByProjectId(project_id);
		List<GenTaskBySqlBuilder> autoTasks = SpringBeanGetter.getDaoBySqlBuilder()
	...
	public List<ApproveTask> getMyApproveTask(@Context HttpServletRequest request, @QueryParam("rand") String rand) {
		...
		while (ite.hasNext()) {
			ApproveTask task = ite.next();
			String create_user_name = SpringBeanGetter.getDaoOfLoginUser().getUserById(task.getCreate_user_id())
					.getUserName();
	...
	public List<ApproveTaskDetail> getMyApproveTaskDetail(@QueryParam("rand") String rand,
			@QueryParam("taskId") String taskId, @QueryParam("taskType") String taskType) {
